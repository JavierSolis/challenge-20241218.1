...
describe('GildedRose', function () {
  describe('basics', function () {
    ...
    it('"Sulfuras"( failed, changed of code: \'Sulfuras, Hand of Ragnaros\' ), being a legendary item, never has to be sold or decreases in Quality', function () {
      ...
      const item = {name:"Sulfuras, Hand of Ragnaros", sellIn:1, quality:20} as Item;
      const gildedRose = new GildedRose([ item ]);
      gildedRose.updateQuality();
      assert.equal(gildedRose.items[0].quality,20,'1st attempt');
      gildedRose.updateQuality();
      assert.equal(gildedRose.items[0].quality,20,'2nd attempt');
    })
